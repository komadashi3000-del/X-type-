...
<p>キーワード: 「浅い納得で終わらない」「問いを生きる形へ返す」</p>
  ...
  const respondentGenderCustomInput = document.getElementById("respondent-gender-custom");
const genderCustomWrap = document.getElementById("gender-custom-wrap");
  respondentGenderInput?.addEventListener("change", () => {
  const isCustom = respondentGenderInput.value === "自認回答";
  genderCustomWrap?.classList.toggle("hidden", !isCustom);
  if (!isCustom && respondentGenderCustomInput) {
    respondentGenderCustomInput.value = "";
  }
});
